...
function display_param(data) {
  ...
  control +=
    " <div class='param param_val " + data.type + selected_class + "'>";
  switch (data.type) {
    case "radio":
  ...
  if (selected_data) {
    ...
    if (
      ...
      switch (selected_param.attr("component")) {
        ...
        default:
          dbugger.print(
            `Component: ${selected_param.attr("component")}`,
            false
          );
      }
...
// function getJSON_from_text(text)
// {
//     return getJson(text)
// }
...
(function ($) {
  ...
  $("#ports").on("click", "td:first-child", function () {
    ...
    port.write("U1\r", function (err) {
      ...
      console.log("Init port message written!");
  ...
  let the_macro = {
    ...
    set_buttons: function () {
      // let macro_exists = this.recorded_macro.length>0;
      //let macro_exists = this.macro_elem.val()>"";
      //console.log("Macro exists: "+this.macro_exists())
      $("#exe_macro").prop("disabled", !this.macro_exists());
    },
    set_text: function () {
  ...
  function receive_data(text) {
    dbugger.print(text, true);
    // data = JSON.parse(text)
    data = text;
    // data = await res.json();
    // data = await getJson(text)
    // data = getJSON_from_text(text)
    // (async () => {
    //     console.log('before start');

    //     data = await getJSON_from_text(text)
  ...
  $("#refresh_button img")
    .mousedown(function () {
      $(this).attr("src", "img/refresh_active.png");
    })
    .mouseup(function () {
      $(this).attr("src", "img/refresh.png");
    });
